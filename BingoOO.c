...
void gerartipo1()
{
    ...
    for(x=0;x<quant;x++)
	{
        ...
        file[x] = fopen(nomecartela[x], "w");             
	}
 //gera os numeros aleatorios do bingo
...
//compara os numeros da matriz e incrementa contador
    for(y=0; y<quant; y++)
    {
        cont = 0;
        for (i=0; i<5; i++)
        ...
        while (cont > 20)
        {
            for (i=0;i<5 && cont > 20; i++)         
            {
                inicio = i*10+1;
                NumAleatorio = inicio + (rand()%10);
                for(j=0; j<5 && cont > 20; j++)
                {
...
void jogar()
{
    int i, y, j, x, k, contZero, NumAleatorio[55];
    srand((unsigned)time(NULL));

    for (x=0; x<55; x++)
    {
        NumAleatorio[x] = 1+rand()%55;
        for (k=0; k<x; k++)
        {
        ...
        {
            contZero = 0;
            for(i=0; i<5; i++)      // percorre os campos comparando os numeros batidos
            {
                for(j=0; j<5; j++)
                {
            ...
            for(i=0; i<5; i++)    // percorre os campos contadando os zeros
            {
                for(j=0; j<5; j++)
                {
                    if (cartelas[y].M[i][j] == 0)
            ...
            if (contZero == 25) // Verifica o vencedor
            {
                mostrar();
                printf("\n\n\t\t VENCEDOR : %s \n\n", cartelas[y].nome);
                return;
...
int main()
{
	...
	do
	{
		...
		{
			case 1:
				printf("\n\tTIPOS DE BINGO DISPONIVEIS:\n\n\tTIPO 1. O usuario escolhe com qual cartela deseja jogar.\n\n\tTIPO 2. O usuario recebe uma cartela gerada aleatoriamente\n");
				printf("\n\tO vencedor sera aquele que primeiro completar o bingo\n");
				break;
			...
			case 2:
                ...
                do
                {
                    ...
                    scanf("%d", &quant);
                    if (quant<=1)
                        printf("\n\n\tInsira uma quantidade maior que 1!\n");
                }while(quant<2);
                if (tipo==1)
                {
                    gerartipo1();
